...
public class Slider
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, ((Number) this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? TurcoHack.colorManager.getColorWithAlpha(TurcoHack.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue()) : TurcoHack.colorManager.getColorWithAlpha(TurcoHack.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue()));
        TurcoHack.textManager.drawStringWithShadow(this.getName() + " " + ChatFormatting.GRAY + (this.setting.getValue() instanceof Float ? this.setting.getValue() : Double.valueOf(((Number) this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 1.7f - (float) TurcoHackGui.getClickGui().getTextOffset(), -1);
    }
    ...
    public boolean isHovering(int mouseX, int mouseY) {
        for (Component component : TurcoHackGui.getClickGui().getComponents()) {
