...
public class StringButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isListening) {
            TurcoHack.textManager.drawStringWithShadow(this.currentString.getString() + TurcoHack.textManager.getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float) TurcoHackGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        } else {
            TurcoHack.textManager.drawStringWithShadow((this.setting.getName().equals("Buttons") ? "Buttons " : (this.setting.getName().equals("Prefix") ? "Prefix  " + ChatFormatting.GRAY : "")) + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) TurcoHackGui.getClickGui().getTextOffset(), this.getState() ? -1 : -5592406);
        }
    }
