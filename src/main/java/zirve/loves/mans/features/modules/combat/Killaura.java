package zirve.loves.mans.features.modules.combat;

import zirve.loves.mans.TurcoHack;
import zirve.loves.mans.event.events.UpdateWalkingPlayerEvent;
import zirve.loves.mans.features.modules.Module;
import zirve.loves.mans.features.setting.Setting;
import zirve.loves.mans.util.DamageUtil;
import zirve.loves.mans.util.EntityUtil;
import zirve.loves.mans.util.MathUtil;
...
public class Killaura extends Module {
    ...
    private void doKillaura() {
        ...
        int wait = !this.delay.getValue().booleanValue() ? 0 : (int) (DamageUtil.getCooldownByWeapon(mc.player) * (this.tps.getValue().booleanValue() ? TurcoHack.serverManager.getTpsFactor() : 1.0F));
        ...
        if (target == null)
            return;
        if (this.rotate.getValue().booleanValue())
            TurcoHack.rotationManager.lookAtEntity(target);
