...
import zirve.loves.mans.features.setting.Bind;
import zirve.loves.mans.features.setting.EnumConverter;
...
public class ConfigManager implements Util {
    ...
    public String config = "glock/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        TurcoHack.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    TurcoHack.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("glock").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("glock/" + name + "/"))) {
            this.config = "glock/" + name + "/";
        } else {
            this.config = "glock/config/";
        }
        TurcoHack.friendManager.onLoad();
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("glock").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("glock/" + name + "/"));
    }
    ...
    public void saveConfig(String name) {
        ...
        if (!path.exists())
            path.mkdir();
        TurcoHack.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("glock/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("glock", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(TurcoHack.moduleManager.modules);
        this.features.add(TurcoHack.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        TurcoHack.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            TurcoHack.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
