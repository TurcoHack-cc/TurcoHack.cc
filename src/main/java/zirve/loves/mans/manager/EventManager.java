...
import zirve.loves.mans.event.events.*;
...
import zirve.loves.mans.features.modules.misc.PopCounter;
...
public class EventManager extends Feature {
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (!fullNullCheck() && (event.getEntity().getEntityWorld()).isRemote && event.getEntityLiving().equals(mc.player)) {
            TurcoHack.inventoryManager.update();
            TurcoHack.moduleManager.onUpdate();
            if ((HUD.getInstance()).renderingMode.getValue() == HUD.RenderingMode.Length) {
                TurcoHack.moduleManager.sortModules(true);
            } else {
                TurcoHack.moduleManager.sortModulesABC();
            }
        }
    }
    ...
    public void onClientConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        TurcoHack.moduleManager.onLogin();
    }

    @SubscribeEvent
    public void onClientDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        TurcoHack.moduleManager.onLogout();
    }
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        if (fullNullCheck())
            return;
        TurcoHack.moduleManager.onTick();
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (fullNullCheck())
            return;
        if (event.getStage() == 0) {
            TurcoHack.speedManager.updateValues();
            TurcoHack.rotationManager.updateRotations();
            TurcoHack.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            TurcoHack.rotationManager.restoreRotations();
            TurcoHack.positionManager.restorePosition();
        }
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.getStage() != 0)
            return;
        TurcoHack.serverManager.onPacketReceived();
        ...
        if (event.getPacket() instanceof net.minecraft.network.play.server.SPacketTimeUpdate)
            TurcoHack.serverManager.update();
    }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        if (event.isCanceled())
            return;
        mc.profiler.startSection("glock");
        ...
        TurcoHack.moduleManager.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0F);
    ...
    public void renderHUD(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR)
            TurcoHack.textManager.updateResolution();
    }
    ...
    public void onRenderGameOverlayEvent(RenderGameOverlayEvent.Text event) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            TurcoHack.moduleManager.onRender2D(render2DEvent);
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState())
            TurcoHack.moduleManager.onKeyPressed(Keyboard.getEventKey());
    }
        ...
        if (event.getMessage().startsWith(Command.getCommandPrefix())) {
            ...
            try {
                ...
                if (event.getMessage().length() > 1) {
                    TurcoHack.commandManager.executeCommand(event.getMessage().substring(Command.getCommandPrefix().length() - 1));
                } else {
